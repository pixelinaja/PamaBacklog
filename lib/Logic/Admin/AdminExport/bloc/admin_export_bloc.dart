  ...
  Stream<AdminExportState> mapEventToState(
    ...
    if (event is AdminExportStart) {
      ...
      try {
        ...
        final getDirectory = await getExternalStorageDirectory();
        final export =
            File("${getDirectory.path}/${sheetObject.sheetName}.xlsx");
        export.createSync(recursive: true);
        export.writeAsBytesSync(encoded);

        yield AdminExportCompleted(
            msg:
                "File Export Telah Tersimpan di ${getDirectory.path}/${sheetObject.sheetName}.xlsx");
      } catch (e) {
